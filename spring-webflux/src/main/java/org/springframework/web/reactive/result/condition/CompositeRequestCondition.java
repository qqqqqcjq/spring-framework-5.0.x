 ...
 * @since 5.0
 */
// 抽象实现AbstractRequestCondition中重写了equals、hashCode和toString三个方法，有8个子类，除了CompositeRequestCondition外每一个子类表示一种匹配条件。
// 比如，PatternsR-equestCondition使用url做匹配，RequestMethodsRequestCondition使用RequestMet-hod做匹配等。
// CompositeRequestCondition本身并不实际做匹配，而是可以将多个别的RequestCondition封装到自己的一个变量里，
// 在用的时候遍历封装RequestCondition的那个变量里所有的RequestCondition进行匹配，也就是大家所熟悉的责任链模式，
// 这种模式在Spring MVC中非常常见，类名一般是CompositeXXX或者XXXComposite的形式，它们主要的作用就是为了方便调用。
