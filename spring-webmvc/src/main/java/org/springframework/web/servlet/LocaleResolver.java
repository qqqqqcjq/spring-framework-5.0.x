 ...
 */
// ViewResolve得到View需要两个参数：一个是视图名，另一个是Locale。视图名是处理器返回的（或者使用RequestToViewNameTranslator解析的默认视图名），Locale是从哪里来的呢？
// 这就是LocaleResolver要做的事情。LocaleResolver用于从request解析出Locale。Locale在前面已经介绍过，就是zh-cn之类，表示一个区域。
// 有了这个就可以对不同区域的用户显示不同的结果，这就是i18n（国际化）的基本原理，LocaleResolver是i18n的基础。
