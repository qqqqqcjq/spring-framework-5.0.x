 ...
 * @since 3.1
 */
//从继承关系可以看出，这个类既可以处理handler method的参数， 也可以处理handler method的返回值
//可以把request body转换为HttpEntity类型或者RequestEntity类型的参数
//可以把HttpEntity类型或者RequestEntity类型的返回值写进response body
public class HttpEntityMethodProcessor extends AbstractMessageConverterMethodProcessor {
	...
	@Override
    // 它的handleReturnValue方法就不详细说了，相比RequestResponseBodyMethodProcessor，多了如下一些增强：
    // 1、对请求中有Vary的进行特殊处理
    ...
    // 最终，最终。正常情况下：调用父类writeWithMessageConverters()方法
