 ...
 * @since 3.1
 */
//InterceptorRegistration 里面封装了拦截，该拦截器需要拦截的路径列表，该拦截器不能拦截(需要排除)的路径列表, 以及一个路径匹配器
public class InterceptorRegistration {
	 ...
	 * @since 5.0.3
	 */
	//addPathPatterns方法（指定拦截路径，往往使用 "/**"） ，这个方法传入的是一个拦截路径列表
	 ...
	 * @since 5.0.3
	 */
	//指定排除拦截路径，用于登录等部分开放接口 ，这个方法传入的是一个需要排除的路径列表
