 ...
 * @since 3.1
 */
//一个基类，通过使用{@link HttpMessageConverter}从请求体转换为需要的参数类型
//此抽象类并没有实现resolveArgument()这个接口方法，而只是提供了一些protected方法，作为工具方法给子类调用，比如最为重要的这个方法：readWithMessageConverters()就是利用消息转换器解析HttpInputMessage的核心
public abstract class AbstractMessageConverterMethodArgumentResolver implements HandlerMethodArgumentResolver {

    // 默认支持的方法（没有Deleted方法）
    // httpMethod为null 或者 方法不属于SUPPORTED_METHODS 或者没有contendType且没有body 那就返回null
	...
	//保存一个HttpMessageConverter的列表
	protected final List<HttpMessageConverter<?>> messageConverters;

	//保存所有支持的MediaType类型
	protected final List<MediaType> allSupportedMediaTypes;

	//保存RequestResponseBodyAdviceChain
	 ...
	 */
	@Nullable
    //应用List<HttpMessageConverter<?>>和RequestResponseBodyAdviceChain将HttpInputMessage转换为要求的对象(比如@RequestBody后面指定的对象类型)
