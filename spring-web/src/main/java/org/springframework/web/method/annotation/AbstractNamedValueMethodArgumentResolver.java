 ...
 * @since 3.1
 */
// @since 3.1  负责从路径变量、request的parameters、request的headers等中拿到值，使用controller中的action方法参数的name(默认或指定)进行匹配，从而获取值。
// 具体参见AbstractNamedValueMethodArgumentResolver的子类
// （controller中的action方法参数可以指定name、required、默认值等属性）

// 子类需要做如下事：获取controller中的action方法参数的name, 然后匹配得到value

//特别注意的是：默认值可以使用${}占位符，或者SpEL语句#{}
public abstract class AbstractNamedValueMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	public final Object resolveArgument(MethodParameter parameter, @Nullable ModelAndViewContainer mavContainer,
        ...
        // private Object resolveStringValue(String value)方法既会解析占位符，还会解析SpEL表达式(#{})，非常强大
        // 因为此时的 name 可能还是被 #{} 符号包裹, 所以可能需要通过 BeanExpressionResolver 来进行解析

        // 使用实例:  Object test(@Value("#{T(Integer).parseInt('${test.myage:10}') + 10}") int myAge)
        //           走到这里 :
        //           namedValueInfo.name是@Value, 所以117行Object resolvedName = resolveStringValue(namedValueInfo.name);执行后还是@Value
        //           namedValueInfo.defaultValue 是 #{T(Integer).parseInt('${test.myage:10}') + 10}的解析是通过， 所以136行arg = resolveStringValue(namedValueInfo.defaultValue);执行后就是28
		...
		if (arg == null) {
            ...
            // 若 arg == null && defaultValue == null && 非 optional 类型的参数 则通过 handleMissingValue 来进行处理
        ...
        // 唯独只有PathVariableMethodArgumentResolver把解析处理的值存储一下数据到
        // HttpServletRequest.setAttribute中（SCOPE_REQUEST 若key已经存在也不会存储了）
	...
	private NamedValueInfo getNamedValueInfo(MethodParameter parameter) {
		...
		if (namedValueInfo == null) {
            ...
            // 都处理好后：new NamedValueInfo(name, info.required, defaultValue);（相当于把注解解析成了此对象嘛~~）
