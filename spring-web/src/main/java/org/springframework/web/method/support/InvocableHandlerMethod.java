...
/**
 * 提供一个方法，用于在通过已注册的{@link HandlerMethodArgumentResolver}解析其方法参数值后调用给定请求的处理程序方法。
 *
...
public class InvocableHandlerMethod extends HandlerMethod {
	...
	@Nullable
    //dataBinderFactory用来创建DataBinder
	 ...
	 */
	@Nullable
    //进入这个方法去执行controller的action方法，获取结果
    //抛出异常时ExceptionHandlerExceptionResolver.doResolveHandlerMethodException#invokeAndHandle#invokeForRequest才提供了Object... providedArgs参数
	public Object invokeForRequest(NativeWebRequest request, @Nullable ModelAndViewContainer mavContainer,
			Object... providedArgs) throws Exception {

	    //获取请求的参数，在这个方法里面完成类型转换等等
		Object[] args = getMethodArgumentValues(request, mavContainer, providedArgs);
		if (logger.isTraceEnabled()) {
			logger.trace("Invoking '" + ClassUtils.getQualifiedMethodName(getMethod(), getBeanType()) +
					"' with arguments " + Arrays.toString(args));
		}
		//使用反射执行controller中的action方法，得到返回结果
	 ...
	 */
    //抛出异常时ExceptionHandlerExceptionResolver.doResolveHandlerMethodException#invokeAndHandle#invokeForRequest#getMethodArgumentValues才提供了Object... providedArgs参数
    private Object[] getMethodArgumentValues(NativeWebRequest request, @Nullable ModelAndViewContainer mavContainer,
			Object... providedArgs) throws Exception {

        //controller中的action方法的参数的类信息会封装成MethodParameter[]，保存在HandlerMethod的MethodParameter[] parameters属性中
        MethodParameter[] parameters = getMethodParameters();
        //最终的参数的保存在数组args
		Object[] args = new Object[parameters.length];
		//循环处理 ： 根据MethodParameter和request中传来的参数，经过类型转换等处理，得到最终的参数
        //对每一个参数的处理，都会创建一个新的DataBinder。 从DataBinder的继承关系和源码可以看出，new的DataBinder里面也会创建新的PropertyEditorRegistry  PropertyEditor Formatter等属性，PropertyEditor Formatter是不共用的
	 ...
	 */
	@Nullable
    //抛出异常时ExceptionHandlerExceptionResolver.doResolveHandlerMethodException#invokeAndHandle#invokeForRequest#getMethodArgumentValues#resolveProvidedArgument才提供了Object... providedArgs参数
