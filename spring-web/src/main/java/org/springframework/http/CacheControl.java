...
public class CacheControl {
	...
	public String getHeaderValue() {
		...
		if (this.maxAge != -1) {
			appendDirective(ccValue, "max-age=" + this.maxAge);
		}
		if (this.noCache) {
		...
		if (this.sMaxAge != -1) {
			appendDirective(ccValue, "s-maxage=" + this.sMaxAge);
		}
		if (this.staleIfError != -1) {
			appendDirective(ccValue, "stale-if-error=" + this.staleIfError);
		}
		if (this.staleWhileRevalidate != -1) {
			appendDirective(ccValue, "stale-while-revalidate=" + this.staleWhileRevalidate);
		}
