 ...
 */
//它是PropertyEditorRegistry接口的实现，提供对defaultEditors和customEditors的管理，最终主要为BeanWrapperImpl和DataBinder服务。
public class PropertyEditorRegistrySupport implements PropertyEditorRegistry {
    ...
    // propertyPath ： 比如IndexedTestBean对象中有一个属性List<TestBean>， 属性TestBean里面有一个name属性
    // IndexedTestBean bean = new IndexedTestBean();
    // BeanWrapper bw = new BeanWrapperImpl(bean);
    // bw.registerCustomEditor(String.class, "list[0].name", PropertyEditor);
    // 这样我们就为bw中的IndexedTestBean bean的属性List<TestBean>[0]的name属性注册了一个属性编辑器
