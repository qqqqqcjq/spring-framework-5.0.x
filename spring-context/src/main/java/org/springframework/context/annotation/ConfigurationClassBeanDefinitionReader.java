...
class ConfigurationClassBeanDefinitionReader {
    ...
    private void loadBeanDefinitionsForConfigurationClass(
		...
		//处理@ImportResource引入的xml配置bean， 我们也可以import xml形式的配置文件描述bean,在这里处理，会使用XmlBeanDefinitionReader
		loadBeanDefinitionsFromImportedResources(configClass.getImportedResources());

		// 注册@Import引入的ImportBeanDefinitionRegistrar，ImportBeanDefinitionRegistrar实现类通过实现ImportBeanDefinitionRegistrar.registerBeanDefinitions引入需要的bean
        // Spring boot大量使用这个功能实现各种@Enable, 比如：
	...
	private void loadBeanDefinitionsForBeanMethod(BeanMethod beanMethod) {
		...
		/*
		如果加上static ,即使有@configuration,也会创建2个对象,就是因为在这里进行了处理
		@Bean
		public static IndexDaoImpl1 indexDaoImpl1(){
		...
		// Consider scoping
        // @bean 在获取bd的时候，也就是在这里，处理bean 的 scope范围
	 ...
	 */
	//@Configuration注解的配置类对应的bd类型是AnnotatedGenericBeanDefinition
    //配置类里面@bean注解引入的bean对应的bd就是 ConfigurationClassBeanDefinition
