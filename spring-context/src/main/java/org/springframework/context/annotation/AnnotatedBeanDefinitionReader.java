...
public class AnnotatedBeanDefinitionReader {
	 ...
	 *  可以证明我们可以看到AnnotationConfigApplicationContext的类关系：
...
//		Assert.notNull(environment, "Environment must not be null");
		...
		//new一个处理@Conditionanl注解的内部类ConditionEvaluator
		this.conditionEvaluator = new ConditionEvaluator(registry, environment, null);
		/*给beanfactory.dependencyComparator new一个AnnotationAwareOrderComparator赋值
		  给 beanfactory.autowireCandidateResolver new一个ContextAnnotationAutowireCandidateResolver赋值
		  这个方法往beanfactory.bdmap中注册了6到8个spring内部bean的beandefinition
		 */
	 ...
	 * @since 5.0
	 */
	//注册一个指定的类的bd到bdmap, 从类声明的注解中的到它的元数据
	<T> void doRegisterBean(Class<T> annotatedClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		/**
		 * 判断这个类是否需要跳过, 通过代码可以知道spring判断是否跳过解析，主要判断@Condition注解信息是否满足条件
		 */
		if (this.conditionEvaluator.shouldSkip(abd.getMetadata())) {
			return;
		}

		//不知道， Supplier是java8的新特性，后面有时间再研究这个方法
		...
		/**
		 * 处理类当中的通用注解(这里的处理是指得到注解信息)
		 * 分析源码可以知道他主要处理
		 * Lazy DependsOn Primary Role等等注解
		 * 处理完成之后processCommonDefinitionAnnotations中依然是把他添加到数据结构(abd)当中
		 *
		 */
		 ...
		 * AnnotatonConfigApplicationContext在初始化的時候通過調用父類的構造方法實例化了一个DefaultListableBeanFactory
