...
/*
 * 当应用比较大的时候，如果所有的内容都当在一个配置文件中，就会显得比较臃肿，同时也不太好理解和维护，
 * 此时可以将一个文件拆分为多个，使用 @PropertySource 注解加载指定的配置文件。
 *
 * 下面演示使用 @PropertySource 注解 加载类路径下的 user.properties 配置文件，
 * @ConfigurationProperties(prefix = "user") 为 User.java POJO 对象的属性赋值。
 *
 * @PropertySource(value = {"classpath:user.properties"})
 * @Component
 * @ConfigurationProperties(prefix = "user")  //这个是spring boot中引入的注解
 * public class User {
 *     private Integer id;
