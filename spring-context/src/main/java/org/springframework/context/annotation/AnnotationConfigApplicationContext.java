...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	public AnnotationConfigApplicationContext() {
		...
		/*
		直接使用DefaultListableBeanFactory作为register(通过ClassPathBeanDefinitionScanner)
		对于config类声明的扫描路径里的类，则是annotationConfigApplicationContext-->refresh()-->beanFactory(DefaultListableBeanFactory)作为register再构造一个ClassPathBeanDefinitionScanner-->调用register注册到beanDefinitionMap中
		AnnotationConfigApplicationContext()构造方法中new的ClassPathBeanDefinitionScanner scanner，传递的register是AnnotationConfigApplicationContext，实际上我们扫描包工作不是scanner这个对象来完成的
		是spring自己new的一个ClassPathBeanDefinitionScanner，这里的scanner仅仅是为了程序员能够在外部调用AnnotationConfigApplicationContext对象的scan方法
		 */
	...
	public AnnotationConfigApplicationContext(Class<?>... annotatedClasses) {
		...
		//父类构造方法中给beanFactory = new new DefaultListableBeanFactory();

		//(调用本类的默认构造函数，也就是无参构造函数时，会默认先调用父类的构造函数)
		//在自己构造方法中初始一个读取器和扫描器
		this();

		//this.reader.register(annotatedClasses) 可以注册一个configuration类，也可以注册一个普通类
