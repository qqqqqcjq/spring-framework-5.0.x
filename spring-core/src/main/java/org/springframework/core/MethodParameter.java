 ...
 */
//封装方法参数信息的帮助类，比如{@link Method} 或者{@link Constructor}的方法参数，以及泛型相关信息。用作传递的规范对象
//封装了java.lang.reflect.Parameter对象
//从4.2起，有一个{@link org.springframework.core.annotation.SynthesizingMethodParameter}子类，可以合成使用属性别名的注解。该子类尤其是用于web和消息端点处理。
public class MethodParameter {
	...
	//当前Method或构造函数对象
    //Executable类是Method和Constructor的父类，一个抽象类，直接继承AccessibleObject类，并实现Membet与GenericDeclaration接口，主要作用是实现类型变量的相关操作以及展示方法的信息。
	...
	//getGenericParameterTypes和getParameterTypes两个方法的区别
    //这两个方法都是用来获取方法形参类型的。
    //getGenericParameterTypes：返回Type类型的数组 Type[].
    //getParameterTypes：返回Class类型的数组： Class<?>[],不包括其他Type

    @Nullable
    //参数可能是Class类型或者其他Type(泛型)类型
