 ...
 */
//基于java 标准反射实现
//从继承关系可以看到，AnnotationMetadataReadingVisitor具有ClassMetadata和AnnotationMeatdata两种功能
//ClassMetadata的实现部分就交给此父类StandardClassMetadata，自己只关注于AnnotationMetadata接口的实现。
public class StandardAnnotationMetadata extends StandardClassMetadata implements AnnotationMetadata {
	...
	@Override
    //获取注解的源注解 ： Class<? extends Annotation> annotationType()
    // 注意返回的是Class类型，注解也是Class类型，不是下面的AnnotatedType， AnnotatedType#getType返回的是注解所注解的元素类型，Type包括数组类型，参数化类型，类型变量或通配符类型
