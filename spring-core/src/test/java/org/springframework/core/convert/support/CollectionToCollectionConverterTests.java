...
public class CollectionToCollectionConverterTests {
    ...
    @Test
    @SuppressWarnings("unchecked")
    public void objectToCollection1() throws Exception {
        Date date = new Date();
        List<Date> list = new ArrayList<Date>() ;
        conversionService.addConverter(new ObjectToCollectionConverter(conversionService));
        //conversionService.addConverter(new CollectionToObjectConverter(conversionService));
        TypeDescriptor sourceType = TypeDescriptor.forObject(date);
        TypeDescriptor targetType =  TypeDescriptor.forObject(list);
        assertTrue(conversionService.canConvert(sourceType, targetType));
        List<Date> result = (List<Date>) conversionService.convert(date, sourceType, targetType);
